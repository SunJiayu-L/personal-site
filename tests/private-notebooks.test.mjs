...
test('only direct opted-in pages render; withdrawing a marker removes the note', async () => {
  ...
  const blogs = await readNotebook(request, course, { public: 'research-post' }, { kind: 'blog', category: 'research', descriptions: { public: 'Summary' } })
  assert.equal(blogs[0].entry.kind, 'blog')
  assert.equal(blogs[0].entry.category, 'research')
  assert.equal(blogs[0].entry.description, 'Summary')
  assert.equal(blogs[0].entry.course, undefined)
  await assert.rejects(readNotebook(request, course, {}, { kind: 'blog', category: 'invalid' }), /configuration/)
  ...
  assert.deepEqual(await readNotebook(request, course, {}, { kind: 'blog', category: 'research' }), [])
})
